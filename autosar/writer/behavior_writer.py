...
class XMLBehaviorWriter(ElementWriter):
    ...
    def writeInternalBehaviorXML(self,internalBehavior):
        ...
        lines = list(
            (
                '<%s>' % internalBehavior.tag(self.version),
                self.indent(
                    '<SHORT-NAME>%s</SHORT-NAME>' % internalBehavior.name, 1
                ),
            )
        )
        ...
        if isinstance(internalBehavior, autosar.behavior.SwcInternalBehavior) and len(internalBehavior.dataTypeMappingRefs)>0:
            ...
            for ref in internalBehavior.dataTypeMappingRefs:
                ...
                if dataTypeMapping is None:
                    raise ValueError(f'Invalid DataTypeMapping reference: {ref}')
        ...
        lines.extend(
            (
                self.indent(
                    '<SUPPORTS-MULTIPLE-INSTANTIATION>%s</SUPPORTS-MULTIPLE-INSTANTIATION>'
                    % ('true' if internalBehavior.multipleInstance else 'false'),
                    1,
                ),
                '</%s>' % internalBehavior.tag(self.version),
            )
    ...
    def _writeRunnableXML(self,runnable):
        ...
        if len(runnable.exclusiveAreaRefs)>0:
            ...
            for exclusiveAreaRef in runnable.exclusiveAreaRefs:
                ...
                if exclusiveArea is None:
                    raise ValueError(f'invalid reference:{exclusiveAreaRef}')
            ...
            lines.append(self.indent('</CAN-ENTER-EXCLUSIVE-AREA-REFS>',1))
        if self.version >= 4.0 and runnable.minStartInterval is not None:
        ...
        lines.extend(
            (
                self.indent('<SYMBOL>%s</SYMBOL>' % runnable.symbol, 1),
                '</RUNNABLE-ENTITY>',
            )
        )
    ...
    def _writeDataReceivePointXML(self, ws, dataReceivePoint):
        lines = ['<%s>' % dataReceivePoint.tag(self.version)]
    ...
    def _writeAutosarVariableRefXML(self, ws, autosarVariableRef):
        lines = ['<%s>' % autosarVariableRef.tag(self.version)]
    ...
    def _writeDataElementInstanceRefXML(self, ws, portRef, dataElemRef):
        ...
        if port is None:
            raise ValueError(f'Invalid port reference: {portRef}')
        dataElement = ws.find(dataElemRef)
        if dataElement is None:
            raise ValueError(f'Invalid port reference: {dataElemRef}')
        if self.version >= 4.0:
            lines.extend(
                (
                    '<ACCESSED-VARIABLE>',
                    self.indent('<AUTOSAR-VARIABLE-IREF>', 1),
                    self.indent(
                        '<PORT-PROTOTYPE-REF DEST="%s">%s</PORT-PROTOTYPE-REF>'
                        % (port.tag(self.version), port.ref),
                        2,
                    ),
                    self.indent(
                        '<TARGET-DATA-PROTOTYPE-REF DEST="%s">%s</TARGET-DATA-PROTOTYPE-REF>'
                        % (dataElement.tag(self.version), dataElement.ref),
                        2,
                    ),
                    self.indent('</AUTOSAR-VARIABLE-IREF>', 1),
                    '</ACCESSED-VARIABLE>',
            ...
            lines.extend(
                (
                    self.indent(
                        '<DATA-ELEMENT-PROTOTYPE-REF DEST="%s">%s</DATA-ELEMENT-PROTOTYPE-REF>'
                        % (dataElement.tag(self.version), dataElement.ref),
                        1,
                    ),
                    '</DATA-ELEMENT-IREF>',
    ...
    def _writeModeGroupInstanceRefXML(self, ws, modeGroupInstanceRef):
        ...
        * Otherwise: <MODE-GROUP-IREF>
        ...
        return list(
            (
                '<{0}>'.format(modeGroupInstanceRef.tag(self.version)),
                self.indent(
                    '<{0} DEST="{1}">{2}</{0}>'.format(
                        portReftag, port.tag(self.version), port.ref
                    ),
                    1,
                ),
                self.indent(
                    '<TARGET-MODE-GROUP-REF DEST="MODE-DECLARATION-GROUP-PROTOTYPE">%s</TARGET-MODE-GROUP-REF>'
                    % (modeGroup.ref),
                    1,
                ),
                '</{0}>'.format(modeGroupInstanceRef.tag(self.version)),
    ...
    def _writeServerCallPointXML(self, ws, runnable, callPoint):
        if not isinstance(callPoint, autosar.behavior.SyncServerCallPoint):
            raise NotImplementedError(type(callPoint))
        lines = list(
            (
                '<SYNCHRONOUS-SERVER-CALL-POINT>',
                self.indent('<SHORT-NAME>%s</SHORT-NAME>' % callPoint.name, 1),
            )
        )
    ...
    def _writeParameterAccessPointXML(self, ws, parameterAccessPoint):
        ...
        lines = ['<%s>' % parameterAccessPoint.tag(self.version)]
    ...
    def _writePortAPIOption(self, ws,option):
        ...
        if port is None:
            raise ValueError(f'invalid reference: {option.portRef}')
    ...
    def _writeEventXML(self, ws, event):
        ...
        lines = list(
            (
                '<%s>' % tag,
                self.indent('<SHORT-NAME>%s</SHORT-NAME>' % event.name, 1),
            )
        )
        ...
        if ws.version >= 4.0:
            if event.disabledInModes is not None:
                ...
                lines.append(self.indent('</DISABLED-MODE-IREFS>',1))
        elif event.modeDependency is not None:
            lines.extend(
                (
                    self.indent('<MODE-DEPENDENCY>', 1),
                    self.indent('<DEPENDENT-ON-MODE-IREFS>', 2),
                )
            )
    ...
    def _writeModeInstanceRefXML(self,ws,modeInstRef):
        ...
        if self.version >= 4.0:
            lines.extend(
                (
                    self.indent(
                        '<CONTEXT-PORT-REF DEST="R-PORT-PROTOTYPE">%s</CONTEXT-PORT-REF>'
                        % (port.ref),
                        1,
                    ),
                    self.indent(
                        '<CONTEXT-MODE-DECLARATION-GROUP-PROTOTYPE-REF DEST="MODE-DECLARATION-GROUP-PROTOTYPE">%s</CONTEXT-MODE-DECLARATION-GROUP-PROTOTYPE-REF>'
                        % (modeDeclarationGroup.ref),
                        1,
                    ),
                    self.indent(
                        '<TARGET-MODE-DECLARATION-REF DEST="%s">%s</TARGET-MODE-DECLARATION-REF>'
                        % (modeDeclaration.tag(self.version), modeDeclaration.ref),
                        1,
                    ),
                )
            )
            ...
            lines.extend(
                (
                    self.indent(
                        '<R-PORT-PROTOTYPE-REF DEST="R-PORT-PROTOTYPE">%s</R-PORT-PROTOTYPE-REF>'
                        % (port.ref),
                        1,
                    ),
                    self.indent(
                        '<MODE-DECLARATION-GROUP-PROTOTYPE-REF DEST="%s">%s</MODE-DECLARATION-GROUP-PROTOTYPE-REF>'
                        % (
                            modeDeclarationGroup.tag(self.version),
                            modeDeclarationGroup.ref,
                        ),
                        1,
                    ),
                    self.indent(
                        '<MODE-DECLARATION-REF DEST="%s">%s</MODE-DECLARATION-REF>'
                        % (modeDeclaration.tag(self.version), modeDeclaration.ref),
                        1,
                    ),
                )
    ...
    def _writeOperationInstanceRefXML(self, ws, parent, operationIRef):
        lines = ['<%s>' % operationIRef.tag(self.version)]
        ...
        lines.extend(
            (
                self.indent(
                    '<%s DEST="%s">%s</%s>'
                    % (portRefTag, port.tag(self.version), port.ref, portRefTag),
                    1,
                ),
                self.indent(
                    '<%s DEST="%s">%s</%s>'
    ...
    def _writeDataInstanceRefXML(self, ws, parent, dataIRef):
        lines = ['<%s>' % dataIRef.tag(self.version)]
    ...
    def _writePerInstanceMemoryXML(self, ws, memory):
        lines = ['<%s>' % memory.tag(self.version)]
    ...
    def _writeServiceNeedsXML(self, ws, elem):
        ...
        tmp = self.writeDescXML(elem)
        if tmp is not None: lines.extend(self.indent(tmp,1))
        if not isinstance(elem, autosar.behavior.NvmBlockServiceNeeds):
    ...
    def _writeRoleBasedDataAssignmentXML(self, ws, elem):
        ...
        if elem.localVariableRef is not None:
            ...
            if localVariable is None:
                raise ValueError(f'Invalid reference: {elem.localVariableRef}')
        else:
            localVariable = None
        if elem.localParameterRef is not None:
            localParameter = ws.find(elem.localParameterRef)
            if localParameter is None:
                raise ValueError(f'Invalid reference: {elem.localParameterRef}')
        else:
            localParameter = None
        lines = list(
            (
                '<%s>' % elem.tag(self.version),
                self.indent('<ROLE>%s</ROLE>' % elem.role, 1),
            )
        )
        ...
        if localVariable is not None:
            lines.extend(
                (
                    self.indent('<USED-DATA-ELEMENT>', 1),
                    self.indent(
                        '<LOCAL-VARIABLE-REF DEST="%s">%s</LOCAL-VARIABLE-REF>'
                        % (localVariable.tag(self.version), localVariable.ref),
                        2,
                    ),
                    self.indent('</USED-DATA-ELEMENT>', 1),
                )
        ...
        if localParameter is not None:
            lines.extend(
                (
                    self.indent('<USED-PARAMETER-ELEMENT>', 1),
                    self.indent(
                        '<LOCAL-PARAMETER-REF DEST="%s">%s</LOCAL-PARAMETER-REF>'
                        % (localParameter.tag(self.version), localParameter.ref),
                        2,
                    ),
                    self.indent('</USED-PARAMETER-ELEMENT>', 1),
                )
    ...
    def _writeRoleBasedPortAssignmentXML(self, ws, elem):
        ...
        if port is None:
            raise ValueError(f'Invalid reference: {elem.portRef}')
        return list(
            (
                "<%s>" % elem.tag(self.version),
                self.indent(
    ...
    def _writeParameterDataPrototype(self, ws, elem):
        datatype = ws.find(elem.typeRef)
        if datatype is None:
            raise ValueError(f'invalid type reference: {elem.typeRef}')
        lines = list(
            (
                '<%s>' % elem.tag(self.version),
        ...
        if elem.swAddressMethodRef is not None or elem.swCalibrationAccess is not None:
            ...
            lines.extend(
                (
                    self.indent('</SW-DATA-DEF-PROPS>', 1),
                    self.indent(
                        '<TYPE-TREF DEST="%s">%s</TYPE-TREF>'
                        % (datatype.tag(self.version), datatype.ref),
                        1,
                    ),
                )
            )
    ...
    def writeNvBlockDescriptorXML(self, nvBlockDescriptor):
        ...
        lines = list(
            (
                '<%s>' % nvBlockDescriptor.tag(self.version),
                self.indent(
                    '<SHORT-NAME>%s</SHORT-NAME>' % nvBlockDescriptor.name, 1
                ),
            )
        )
