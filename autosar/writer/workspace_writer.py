...
class WorkspaceWriter(BaseWriter):
    ...
    def beginFile(self):
        ...
        if (self.version >= 3.0) and (self.version < 4.0):
            ...
            schema = f'autosar_{versionString}.xsd' if self.schema is None else self.schema
            lines.extend(
                (
                    '<AUTOSAR xsi:schemaLocation="http://autosar.org/%s %s" xmlns="http://autosar.org/%s" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">'
                    % (versionString, schema, versionString),
                    f'{self.indentChar}<TOP-LEVEL-PACKAGES>',
                )
            )
            ...
            lines.extend(
                (
                    ...
                    '<AUTOSAR xmlns="http://autosar.org/schema/r4.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://autosar.org/schema/r4.0 AUTOSAR_{0}-{1}-{2}.xsd">'.format(
                        self.major, self.minor, self.patch
                    ),
                    f'{self.indentChar}<AR-PACKAGES>',
                )
            )
    ...
    def endFile(self):
        lines=[]
        if (self.version >= 3.0) and (self.version<4.0):
            lines.append(f'{self.indentChar}</TOP-LEVEL-PACKAGES>')
        else:
            lines.append(f'{self.indentChar}</AR-PACKAGES>')
